...
from metrics import CombinedBoundaryLoss, calculate_dice_score, calculate_iou_score, compute_ece

def train_ultimate_model():
    print("=" * 75)
    print(" HQU-MSANet Ultimate Full Dataset Fine-Tuning & Confidence Optimization")
    ...
    # Load Current Best 95.42% Weights as starting point
    checkpoint_dir = "checkpoints"
    best_model_path = os.path.join(checkpoint_dir, "hqu_msanet_brats2023_ultimate_best.pth")
    previous_best_path = os.path.join(checkpoint_dir, "hqu_msanet_brats2023_best.pth")
    
    starting_best_dice = 0.9542
    if os.path.exists(previous_best_path):
        checkpoint = torch.load(previous_best_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        starting_best_dice = checkpoint.get('val_dice', 0.9542)
    ...
    criterion = CombinedBoundaryLoss(lambda_edge=0.40) # Sharp edge boundary supervision
    scaler = torch.amp.GradScaler('cuda')

    best_val_dice = starting_best_dice

    print("\n" + "-" * 75)
    print(f" Starting Ultimate Confidence Optimization Loop ({epochs} Epochs)...")
    print("-" * 75)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (images, masks, _) in enumerate(train_loader, 1):
            ...
            print(f"\r  [Ultimate - Epoch {epoch:02d}/{epochs:02d}] Step {step:03d}/{total_batches:03d} ({batch_pct:5.1f}%) | Loss: {loss.item():.4f}", end="", flush=True)
        ...
        print(f"\n[+] Ultimate Epoch [{epoch:02d}/{epochs:02d}] Completed in {epoch_time:.2f}s | Overall Progress: {overall_pct:5.1f}%")
        ...
        # Strict checkpointing: Save ONLY if score improves or maintains top baseline
        if avg_val_dice >= best_val_dice:
            ...
            print(f"    --> [SAVED ULTIMATE BEST MODEL] Val Dice: {best_val_dice:.4f} -> {best_model_path}")
        print("-" * 75)

    total_duration = time.time() - start_time
    print("=" * 75)
    print(f" Ultimate Full Model Training Completed Successfully in {total_duration/60:.2f} minutes!")
    print(f" Highest Achieved Validation Dice Coefficient: {best_val_dice:.4f}")
    print(f" Master Single Model Weights File: {os.path.abspath(best_model_path)}")
    print("=" * 75)
