...
def prepare_full_preprocessed_dataset():
    """
    Expands the caching to 300 BraTS 2023 Patients and finer slice steps (step=3)
    for high-density learning and maximum segmentation accuracy.
    """
    ...
    selected_patients = patient_dirs[:300]
    ...
    for idx, p_dir in enumerate(selected_patients):
        ...
        try:
            ...
            # Extract slices step=3
            for slice_idx in range(45, 115, 3):
    ...
    print(f"[+] Done! Cached {count} high-density tensor slices in {time.time() - start:.1f}s.")

if __name__ == "__main__":
    prepare_full_preprocessed_dataset()
